...
def doublestemandleaf():
    ...
    st.write(f"Indeed, truncating each value in your data sets right before the {pos_rep(stem_pos)}s position results in a data set with identical values (and possibly zeros when truncation eats up the entire number):\n")
    st.write([truncate(num,stem_pos) for num in data])
    ...
    for key in sorted(list(allkeys)):
        ...
        st.write((" "*pad) + leafstring1[::-1]+"  |  " + str(key) + "  |  " + leafstring2)
        st.write("")
    ...
    if 0 in stems:
        maxsl=2+max(max(len(("  ".join(dict1[key]))) for key in allkeys),len("  ".join(sml1)))
    else:
    ...
    if (0 in stems) and (len(smallerleaves1)!=0 or len(smallerleaves2)!=0):
        ...
        pad=maxsl-len(leafstring1)
        st.write("  "+(" "*pad) + leafstring1[::-1]+"  |  " + "L"+ " "*(Lpad-1) + "  |  " + leafstring2)
        st.write("")
    ...
    for key in sorted(list(allkeys)):
        ...
        leafstring2=("  ".join(dict2[key]))
        pad=maxsl-len(leafstring1)
        st.write("  "+(" "*pad) + leafstring1[::-1]+"  |  " + fullkey + "  |  " + leafstring2)
        st.write("")
