...
def doublestemandleaf():
    ...
    maxsl=5+max(len(("  ".join(dict1[key]))) for key in allkeys) 
    ...
    if 0 in stems:
        maxsl=3+max(max(len(("  ".join(dict1[key]))) for key in allkeys),len("  ".join(sml1))) #2+
    else:
        maxsl=3+max(len(("  ".join(dict1[key]))) for key in allkeys)
